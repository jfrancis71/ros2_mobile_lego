#include "alfie_hardware/alfie_hardware.hpp"
...
const double traction_gear_ratio = 22.0/14.0;
...
{
hardware_interface::CallbackReturn AlfieHardware::on_init(
...
std::vector<hardware_interface::StateInterface> AlfieHardware::export_state_interfaces()
{
  std::vector<hardware_interface::StateInterface> state_interfaces;
  state_interfaces.emplace_back(hardware_interface::StateInterface(
      "traction_wheel", hardware_interface::HW_IF_VELOCITY, &hw_traction_vel_));
  state_interfaces.emplace_back(hardware_interface::StateInterface(
      "traction_wheel", hardware_interface::HW_IF_POSITION, &hw_traction_position_));
  state_interfaces.emplace_back(hardware_interface::StateInterface(
...
std::vector<hardware_interface::CommandInterface> AlfieHardware::export_command_interfaces()
{
  std::vector<hardware_interface::CommandInterface> command_interfaces;
  command_interfaces.emplace_back(hardware_interface::CommandInterface(
      "traction_wheel", hardware_interface::HW_IF_VELOCITY, &hw_traction_vel_command_));
  command_interfaces.emplace_back(hardware_interface::CommandInterface(
...
hardware_interface::CallbackReturn AlfieHardware::on_activate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  RCLCPP_INFO(rclcpp::get_logger("AlfieHardware"), "Activating ...please wait...");

  RCLCPP_INFO(
    rclcpp::get_logger("AlfieHardware"),
  ...
  for (auto i = 0; i < hw_start_sec_; i++)
  {
    ...
    RCLCPP_INFO(
      rclcpp::get_logger("AlfieHardware"), "%.1f seconds left...", hw_start_sec_ - i);
  }

  RCLCPP_INFO(rclcpp::get_logger("AlfieHardware"), "Successfully activated!");

  return hardware_interface::CallbackReturn::SUCCESS;
}

hardware_interface::CallbackReturn AlfieHardware::on_deactivate(
  const rclcpp_lifecycle::State & /*previous_state*/)
{
  RCLCPP_INFO(rclcpp::get_logger("AlfieHardware"), "Deactivating ...please wait...");
  ...
  for (auto i = 0; i < hw_stop_sec_; i++)
  {
    rclcpp::sleep_for(std::chrono::seconds(1));
    RCLCPP_INFO(
      rclcpp::get_logger("AlfieHardware"), "%.1f seconds left...", hw_stop_sec_ - i);
  }

  RCLCPP_INFO(rclcpp::get_logger("AlfieHardware"), "Successfully deactivated!");

  return hardware_interface::CallbackReturn::SUCCESS;
}

hardware_interface::return_type AlfieHardware::read(
  const rclcpp::Time &, const rclcpp::Duration & period)
{
  ...
  hw_traction_vel_ = (radps_A+radps_D)*0.5/traction_gear_ratio;
  hw_traction_position_ = (position_A+position_D)*0.5/traction_gear_ratio;
...
hardware_interface::return_type AlfieHardware::write(
  const rclcpp::Time &, const rclcpp::Duration & /*period*/)
{
  RCLCPP_DEBUG(
    rclcpp::get_logger("AlfieHardware"),
    "Command interface traction has value %f.",
    hw_traction_vel_command_);
  double dps = hw_traction_vel_command_*360.0/(2.0*MATH_PI);
  double motor_dps = dps * traction_gear_ratio;
  brickpi3.set_motor_dps(PORT_A, motor_dps);
  brickpi3.set_motor_dps(PORT_D, motor_dps);
...
PLUGINLIB_EXPORT_CLASS(
  alfie_hardware::AlfieHardware, hardware_interface::SystemInterface)
